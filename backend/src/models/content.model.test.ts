...
describe("Content Generation Models", () => {
  ...
  beforeEach(() => {
    ...
    mockContentRequest = {
      ...
      previousStories: ["story-1", "story-2"],
      targetDuration: 180,
  ...
  describe("ContentRequest validation", () => {
    ...
    it("should reject ContentRequest with non-array previousStories", () => {
      const invalidRequest = {
        ...mockContentRequest,
        previousStories: "not-array" as any,
      };
      assertFalse(validateContentRequest(invalidRequest));
      assertFalse(isContentRequest(invalidRequest));
    });
  ...
  describe("Helper functions", () => {
    it("should convert GeneratedContent to StoredContent", () => {
      ...
      assertEquals(
        storedContent.estimatedDuration,
        mockGeneratedContent.estimatedDuration
      );
      assertEquals(storedContent.sources, mockGeneratedContent.sources);
      assertEquals(
        storedContent.poiReferences,
        mockGeneratedContent.poiReferences
      );
    ...
    it("should create ContentRequest with default values", () => {
      ...
      assertEquals(request.previousStories, []);
    ...
    it("should create ContentRequest with custom values", () => {
      const previousStories = ["story-1", "story-2"];
      ...
      assertEquals(request.pois, [mockPOI]);
      assertEquals(request.userLocation, mockLocation);
      assertEquals(request.previousStories, previousStories);
  ...
  describe("Serialization and deserialization", () => {
    it("should serialize and deserialize ContentRequest correctly", () => {
      ...
      assertEquals(
        deserialized.targetDuration,
        mockContentRequest.targetDuration
      );
      assertEquals(
        deserialized.previousStories,
        mockContentRequest.previousStories
      );
    });
  ...
  describe("Edge cases", () => {
    ...
    it("should handle whitespace-only strings appropriately", () => {
      ...
      for (const whitespace of whitespaceTests) {
        ...
        assertFalse(
          validateGeneratedContent(invalidContent),
    ...
    it("should validate content with special characters in text fields", () => {
      ...
      for (const narrative of specialCharacterTests) {
        ...
        assert(
          validateGeneratedContent(content),
          `Failed to validate content with special characters: ${narrative.substring(
            0,
            50
          )}...`
        );
