...
describe('Enhanced Highway Detection (Method 4)', () => {
  ...
  describe('Progressive radius search', () => {
    it('should implement progressive radius search for highway detection', async () => {
      ...
      const smallRadius = await service.discoverPOIs(testLocation, { radiusMeters: 500 });
      const largeRadius = await service.discoverPOIs(testLocation, { radiusMeters: 5000 });
      ...
      const smallRadiusHighways = smallRadius.filter(poi => poi.category === POICategory.MAJOR_ROAD);
      const largeRadiusHighways = largeRadius.filter(poi => poi.category === POICategory.MAJOR_ROAD);

      assert(smallRadiusHighways.length > 0, 'Should find highways with small radius');
      assert(largeRadiusHighways.length > 0, 'Should find highways with large radius');
    });
  ...
  describe('Enhanced Overpass query structure', () => {
    it('should handle geometry-based post-processing', async () => {
      ...
      for (const highway of highways) {
        ...
        assert(highway.location.latitude >= -90 && highway.location.latitude <= 90);
        assert(highway.location.longitude >= -180 && highway.location.longitude <= 180);
    ...
    it('should format highway names correctly', async () => {
      ...
      for (const highway of highways) {
        ...
        assert(!highway.name.match(/^[A-Z]-?\d+$/), 
  ...
  describe('Confidence scoring', () => {
    it('should calculate confidence scores for highway detection', async () => {
      ...
      for (const highway of highways) {
        ...
        assert(highway.metadata.significanceScore >= 80, 
    ...
    it('should handle various highway types with appropriate confidence', async () => {
      ...
      const stateHighways = highways.filter(h => h.name.includes('Highway') && !h.name.includes('US Highway'));
  ...
  describe('Error handling and resilience', () => {
    ...
    it('should handle network failures gracefully', async () => {
      ...
      const highways = pois.filter(poi => poi.category === POICategory.MAJOR_ROAD);
      assert(highways.length > 0, 'Should return mock highway data when APIs fail');
    });
  });
  ...
  describe('Integration with existing functionality', () => {
    it('should maintain compatibility with existing POI discovery', async () => {
      ...
      const nonHighwayPois = pois.filter(poi => poi.category !== POICategory.MAJOR_ROAD);
    ...
    it('should work with distance filtering', async () => {
      ...
      const originalHighways = pois.filter(poi => poi.category === POICategory.MAJOR_ROAD);
      const filteredHighways = filteredPois.filter(poi => poi.category === POICategory.MAJOR_ROAD);

      // Some highways should remain after filtering (mock data includes highways at user location)
      if (originalHighways.length > 0) {
        assert(filteredHighways.length > 0, 'Distance filtering should preserve nearby highways');
      }
    });
    ...
    it('should work with significance sorting', async () => {
      ...
      for (let i = 0; i < sortedPois.length - 1; i++) {
        ...
        assert(currentScore >= nextScore, 
          `POIs should be sorted by significance: ${sortedPois[i].name} (${currentScore}) >= ${sortedPois[i + 1].name} (${nextScore})`);
      }
      ...
      const highSignificanceCategories = [POICategory.MAJOR_ROAD, POICategory.TOWN];
      const hasHighSignificance = topCategories.some(cat => highSignificanceCategories.includes(cat));
      assert(hasHighSignificance, 'Top POIs should include high-significance categories like highways or towns');
    });
  });
});