...
describe('Highway Detection Comparison API', () => {
  ...
  beforeEach(() => {
    ...
    controller = new HighwayDetectionController(poiService, googleRoadsService);
  });
  ...
  describe('POST /api/highway/detection-comparison', () => {
    it('should validate input parameters', async () => {
      ...
      try {
        await controller.compareHighwayDetectionMethods({ latitude: 91, longitude: -74 });
      ...
      try {
        await controller.compareHighwayDetectionMethods({ latitude: 40, longitude: 181 });
    ...
    it('should return comparison results for all four methods', async () => {
      ...
      const result = await controller.compareHighwayDetectionMethods(testLocation);
    ...
    it('should handle different test locations from highway-detection-methods.md', async () => {
      const testLocations = [
        { latitude: 40.53383335817636, longitude: -74.3467882397128, name: 'US Route 1' },
        { latitude: 40.7128, longitude: -74.006, name: 'NYC Interstate' },
        { latitude: 37.7749, longitude: -122.4194, name: 'San Francisco' },
      ];

      for (const location of testLocations) {
        const result = await controller.compareHighwayDetectionMethods(location);
        ...
        assert(totalHighways >= 0, `Should return some highway results for ${location.name}`);
      }
    });
    ...
    it('should handle method failures gracefully', async () => {
      ...
      const result = await controller.compareHighwayDetectionMethods(edgeLocation);
      ...
      for (const [methodName, methodResult] of Object.entries(result.methods)) {
        ...
        assert(
          Array.isArray(methodResult.highways) || typeof methodResult.error === 'string',
    ...
    it('should demonstrate accuracy improvements over current method', async () => {
      ...
      const result = await controller.compareHighwayDetectionMethods(onHighwayLocation);
      ...
      if (currentMethod.highways.length > 0 && pointToLineMethod.highways.length > 0) {
