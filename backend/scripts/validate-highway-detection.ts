...
async function testHighwayDetection(
  point: TestPoint
): Promise<ComparisonResult | null> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/highway/detection-comparison`,
      {
        method: "POST",
        headers: {
    ...
    console.error(`❌ Network Error for ${point.expectedLabel}:`, error);
    return null;
  }
}
...
async function runValidation() {
  ...
  for (const point of testPoints) {
    console.log(
      `📍 Testing: ${point.expectedLabel} (${point.latitude}, ${point.longitude})`
    );
    ...
    const accuracy = {
      ...
      pointToLine: checkAccuracy(
        point.expectedLabel,
        result.methods.pointToLine
      ),
      googleRoads: checkAccuracy(
        point.expectedLabel,
        result.methods.googleRoads
      ),
      enhancedOverpass: checkAccuracy(
        point.expectedLabel,
    ...
    console.log(
      `   Current Method:        ${
        accuracy.current ? "✅" : "❌"
      } ${getTopHighways(result.methods.current)}`
    );
    console.log(
      `   Point-to-Line:         ${
        accuracy.pointToLine ? "✅" : "❌"
      } ${getTopHighways(result.methods.pointToLine)}`
    );
    console.log(
      `   Google Roads:          ${
        accuracy.googleRoads ? "✅" : "❌"
      } ${getTopHighways(result.methods.googleRoads)}`
    );
    console.log(
      `   Enhanced Overpass:     ${
        accuracy.enhancedOverpass ? "✅" : "❌"
      } ${getTopHighways(result.methods.enhancedOverpass)}`
    );
    ...
    console.log(
      `      Current: ${
        distances.current ? distances.current.distance.toFixed(0) + "m" : "N/A"
      }`
    );
    console.log(
      `      Point-to-Line: ${
        distances.pointToLine
          ? distances.pointToLine.distance.toFixed(0) + "m"
          : "N/A"
      }`
    );
    console.log(
      `      Google Roads: ${
        distances.googleRoads
          ? distances.googleRoads.distance.toFixed(0) + "m"
          : "N/A"
      }`
    );
    console.log(
      `      Enhanced Overpass: ${
        distances.enhancedOverpass
          ? distances.enhancedOverpass.distance.toFixed(0) + "m"
          : "N/A"
      }`
    );
    console.log("");
  ...
  if (successfulTests > 0) {
    const methodStats = {
      ...
      enhancedOverpass: results.filter((r) => r.accuracy.enhancedOverpass)
        .length,
    };
    ...
    for (const threshold of distanceThresholds) {
      ...
      for (const result of validResults) {
        const distances = {
          ...
          enhancedOverpass: getClosestHighway(
            result.result!.methods.enhancedOverpass
          ),
        };

        if (distances.current && distances.current.distance <= threshold)
          withinThreshold.current++;
        if (
          distances.pointToLine &&
          distances.pointToLine.distance <= threshold
        )
          withinThreshold.pointToLine++;
        if (
          distances.googleRoads &&
          distances.googleRoads.distance <= threshold
        )
          withinThreshold.googleRoads++;
        if (
          distances.enhancedOverpass &&
          distances.enhancedOverpass.distance <= threshold
        )
          withinThreshold.enhancedOverpass++;
      }

      console.log(
        `  Current Method:        ${withinThreshold.current}/${
          validResults.length
        } (${((withinThreshold.current / validResults.length) * 100).toFixed(
          1
        )}%)`
      );
      console.log(
        `  Point-to-Line:         ${withinThreshold.pointToLine}/${
          validResults.length
        } (${(
      ...
      console.log(
        `  Google Roads:          ${withinThreshold.googleRoads}/${
          validResults.length
        } (${(
      ...
      console.log(
        `  Enhanced Overpass:     ${withinThreshold.enhancedOverpass}/${
          validResults.length
        } (${(
    ...
    console.log(
      `  Point-to-Line:         ${
        validCounts.pointToLine > 0
          ? (avgDistances.pointToLine / validCounts.pointToLine).toFixed(1) +
            "m"
          : "N/A"
      }`
    );
    console.log(
      `  Google Roads:          ${
        validCounts.googleRoads > 0
          ? (avgDistances.googleRoads / validCounts.googleRoads).toFixed(1) +
            "m"
          : "N/A"
      }`
    );
    console.log(
      `  Enhanced Overpass:     ${
        validCounts.enhancedOverpass > 0
          ? (
              avgDistances.enhancedOverpass / validCounts.enhancedOverpass
            ).toFixed(1) + "m"
          : "N/A"
      }`
    );
    ...
    if (validResults.length > 0) {
      avgTimes.current =
        validResults.reduce(
          (sum, r) => sum + (r.result?.methods.current.processingTime || 0),
          0
        ) / validResults.length;
      ...
      avgTimes.enhancedOverpass =
        validResults.reduce(
          (sum, r) =>
            sum + (r.result?.methods.enhancedOverpass.processingTime || 0),
          0
        ) / validResults.length;
      ...
      console.log(
        `  Point-to-Line:         ${avgTimes.pointToLine.toFixed(1)}ms`
      );
      console.log(
        `  Google Roads:          ${avgTimes.googleRoads.toFixed(1)}ms`
      );
      console.log(
        `  Enhanced Overpass:     ${avgTimes.enhancedOverpass.toFixed(1)}ms`
      );
    }
  }
...
async function checkServerHealth() {
  try {
    ...
    console.error(
      "❌ Server is not running or not accessible at",
      API_BASE_URL
    );
    console.error(
